...
# M9(2026-08 加):learning_ab 的 A/B 证实,复盘虽已喂进 memory_context,委员会
# 却几乎不权衡它(DiD≈0)——瓶颈是框定太弱("参考、不是硬约束")。这里追加一条
# 针对**该票自己的已平仓结果**的显式权衡要求,尤其是亏损复盘:重复在同一只票上
# 犯同样的错是最该避免的。仍是 advisory(只改委员会怎么推理,绝不进闸门/下单路径),
# 但要求委员会在裁决里正面回应自己的亏损历史。效果用 learning_ab 的 DiD 前后对比验证。
_MEMORY_SECTION_TEMPLATE = (
    "以下是我们已积累的内部知识与该票历史决策,请在分析时参考(但这是历史经验,"
    "不是硬约束):\n{memory_context}\n"
    "特别注意:如果上面包含**我们自己过去在这只股票上的已平仓交易结果**(尤其是"
    "亏损的复盘),必须在裁决中显式回应它——在同一只票上重复犯同样的错是最该避免的。"
    "若我们上次在这只票上亏了钱、这次又要买入,就必须给出**与上次不同的、具体的"
    "理由**说明为什么这次不一样;给不出就更应倾向 hold、并相应压低 confidence。\n"
)
